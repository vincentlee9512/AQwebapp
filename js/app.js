...
////eventlistener that keep tracking the lat,lng for center and bounds(northeast point and southwest point)
        ...
        google.maps.event.addListener(map, "center_changed", function () {
            ...
            console.log("distance from center  calculated: " + distRadius);

            searchBoxElement.placeholder = strHTML;

            datalist = getData(distRadius, centerObj.lat(), centerObj.lng());
        });
        ...
        google.maps.event.addListener(map, "bounds_changed", function () {
            ...
            var distRadius = calcDist(centerObj.lat(), centerObj.lng(), ne.lat(), ne.lng());
            console.log("bounds from NE bound calculated: " + distRadius);
...
////////////////////////////////////////////////////////////////////////////////////////////
        ...
        function getData(radius, centerLat, centerLng){
            var radius = radius;
            var centerLat = centerLat;
            var centerLng = centerLng;
            var datalistHere;

            var url = 'https://api.openaq.org/v1/measurements?coordinates=' + centerLat + ',' + centerLng + '&radius=' + radius;

            $http.get(url).then(function(response) {
                datalistHere = response.data.results;

                console.log(datalistHere);
